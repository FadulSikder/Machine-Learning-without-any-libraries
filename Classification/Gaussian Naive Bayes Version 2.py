...
def mean_variance(df):
    ...
    mean = data_mean[['Height', 'Weight']].to_numpy()

    data_var = df.groupby('Gender').var()
    var = data_var[['Height', 'Weight']].to_numpy()
...
def prior_probability(data):
    data_in_numarr = data[['Height', 'Weight', 'Gender']].to_numpy()
...
def leave_one_out_evaluation():
    ...
    for i in range(loop_var):
        ...
        test_data = dropped_row[['Height', 'Weight']].to_numpy()
