...
def updated_dataset(train_data_weighted, train_pred):
    wrong = np.zeros(len(train_pred))
    sum = 0
    bucket = 0
    indexes = []
    for i in range(len(train_pred)):
        if train_data_weighted[i][3] != train_pred[i]:
            wrong[i] = 1
    weight = train_data_weighted[:,-1]
    total_error = np.sum(weight*wrong)

    pot = 1/2 * math.log((1-total_error)/total_error)
    for i in range(len(wrong)):
      if wrong[i] == 1:
          train_data_weighted[i][4] = train_data_weighted[i][4] + math.exp(pot)
          sum = sum +train_data_weighted[i][4]
      else:
          train_data_weighted[i][4] = train_data_weighted[i][4] + math.exp(-pot)
          sum = sum +train_data_weighted[i][4]
    train_data_weighted[:,4] /= sum
    for i in range(len(wrong)):
        random = np.random.rand()
        for j in range(len(wrong)):
            bucket = bucket +train_data_weighted[j][4]
            if random <= bucket:
                break
        indexes.append(j)
        bucket = 0
    
    train_data = train_data_weighted[indexes]
    X_train = train_data[:,:-2]
    Y_train = train_data[:,-2].reshape(-1,1)
    
    return X_train,Y_train

def boosting(boost_num):

    X_train, Y_train, X_test, Y_test = load_data()
    train_data = np.concatenate((X_train, Y_train), axis=1)
    test_data = np.concatenate((X_test, Y_test), axis=1)
    train_data_weighted = train_data.copy()
    weights = (np.ones(len(train_data)) / len(train_data)).reshape(-1,1)
    train_data_weighted = np.concatenate((train_data_weighted,  weights), axis=1)
    ...
    for i in range(boost_num):
        sample_classifier = DecisionTreeClassifier(min_samples_split=2, max_depth=3)
        tree_root = sample_classifier.fit(X_train,Y_train)
        train_pred = sample_classifier.predict(X_train, tree_root)
        sample_pred = sample_classifier.predict(X_test, tree_root)
        X_train,Y_train   = updated_dataset(train_data_weighted, train_pred)
    ...
    boost_accuracy = accuracy( Y_test.flatten().tolist(),Y_pred)
    return boost_accuracy
...
def main():
    ...
    n_boosting = [10, 25, 50]
    for boost_num in n_boosting:
        boosting_accuracy = boosting(boost_num)
        print('\033[1m' + "Boosting for {} times. Accuracy:{}".format(boost_num, boosting_accuracy)+ '\033[0m')
